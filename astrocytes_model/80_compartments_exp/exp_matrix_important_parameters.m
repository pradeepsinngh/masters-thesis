...
ki2 = 2; % units uM^-1 (multiplies [PKC*])  (k2 in Kang-Othmer)
...
% kCa = 400;   % units uM.s^-1 (multiplies effective open-channel fraction)
...
ng(1) = 100; % number of compartments per morph
...
m = 100; % number of grid compartments; 2*k+1 = min to support O(2) accuracy
...
% xgrid1 = [0, dx/2:dx:50-dx/2, 50]';
 xgrid1 = [0, dx/2:dx:100-dx/2, 100]';
% xgrid2 = [dx/2:dx:50-dx/2]';

Camax = 0;

tend = 4; % end time ( t(start) = 0)
